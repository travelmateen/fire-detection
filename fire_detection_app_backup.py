...
if source == "Video":
    if uploaded_files is not None:
        ...
        if 'video_file' not in st.session_state or st.session_state.get('current_video') != uploaded_files.name:
        ...
        if st.session_state.get("start_video_processing", False):
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.mp4') as tmp_file:
                tmp_file.write(st.session_state.video_file.read())
            ...
            try:
                while (st.session_state.get("start_video_processing", False) and
                       st.session_state.get("source_selection") == "Video"):
                    ret, frame = cap.read()
                    if not ret:
                        break
                    ...
                    # Alert for high confidence detections (original way)
            ...
            for i, uploaded_file in enumerate(images_to_process):
                ...
                try:
                    ...
                    if show_stats:
                        ...
                        # Alert - Use >= for alert threshold (original way)
